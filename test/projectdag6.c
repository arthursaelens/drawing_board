...
float interval_op = 0.9;
float interval_neer = 0.4; //uiterst links is 0.5(pen neer) en uiterst rechts is 2.3(pen op), midden wellicht 1.5
...
ISR(TIMER3_COMPA_vect){
    ...
    if (state == 2){
        if (servo3 == 1) {
            ...
            OCR3A += interval_op * ms_to_ticks_fact; //tel geschaalde versie v interval 2 op voor nieuwe timelimit(puls voor servo 2)
        } else {
            OCR3A += interval_neer * ms_to_ticks_fact; //tel geschaalde versie v interval 2 op voor nieuwe timelimit(puls voor servo 2)
        }
...
int main(void) {
    ...
    //Point_XY *cirkel_lijst_pointer = generatePointsOnCircle(1,12,0,4);
    

    Point_XY array_XY[] =  {{8,0},{8,0},{8.5,0},{8.5,-1},{9,-1},{9,0},{10,0},{10,-2},{9,-3},{8,-2},{8,0},{10,0},{10,-2},{8,-2}};
    
    Point_XY *array_XY2 = generatePointsOnCircle(1,12,0,4);
    /*for (int i = 0; i<4;i++) {
        array_XY2[i] = cirkel_lijst_pointer[i];
    }*/
    
    //{{8,2},{8,2},{10,2},{10,1},{8,1},{8,2},{10,2},{10,1},{8,1}};
